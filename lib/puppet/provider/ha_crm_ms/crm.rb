...
Puppet::Type.type(:ha_crm_ms).provide(:crm) do
  ...
  def create
    ...
    params << "meta"
    [:priority, :target_role, :is_managed, :clone_max, :clone_node_max, :notify_clones, :globally_unique, :ordered, :interleave, :master_max, :master_node_max].each do |attr|
      attr_name = (attr == :notify_clones) ? "notify" : attr.to_s
      params << "#{attr_name}=#{resource[attr]}" if resource[attr].to_s != "absent"
    end
    crm *params
  end
  ...
  def master_max
    if resource[:only_run_on_dc] && !(Facter.value(:ha_cluster_dc) == Facter.value(:fqdn) || Facter.value(:ha_cluster_dc) == Facter.value(:hostname)) 
      ...
      cib = REXML::Document.new File.open("/var/lib/heartbeat/crm/cib.xml")
      resources = REXML::XPath.first(cib, "//cib/configuration/resources")
      nvpair = REXML::XPath.first(resources, "//master[@id='#{resource[:id]}']/meta_attributes/nvpair[@name='master-max']")
      if nvpair.nil?
        :absent
      else
        nvpair.attribute(:value).value
  ...
  def master_node_max
    if resource[:only_run_on_dc] && !(Facter.value(:ha_cluster_dc) == Facter.value(:fqdn) || Facter.value(:ha_cluster_dc) == Facter.value(:hostname)) 
      resource[:master_node_max]
    else
      cib = REXML::Document.new File.open("/var/lib/heartbeat/crm/cib.xml")
      resources = REXML::XPath.first(cib, "//cib/configuration/resources")
      nvpair = REXML::XPath.first(resources, "//master[@id='#{resource[:id]}']/meta_attributes/nvpair[@name='master-node-max']")
      if nvpair.nil?
        :absent
      else
        nvpair.attribute(:value).value
